...
test('the health endpoint reports the running version while SQLite is usable', async () => {
  ...
  try {
    ...
    assert.deepEqual(health, { status: 'ok', database: 'ok', version: expected, ready: true, restoring: false, resetting: false, critical: false });

    // Deployment scripts poll this endpoint, so it must not leak paths or other diagnostics.
    assert.deepEqual(Object.keys(health).sort(), ['critical', 'database', 'ready', 'resetting', 'restoring', 'status', 'version']);
  } finally {
    if (server) await stopServer(server);
    await rm(dataDir, { recursive: true, force: true });
  }
