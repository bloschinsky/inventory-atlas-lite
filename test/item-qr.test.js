...
test('an item UUID is encoded as the canonical ial:item:v1 payload', () => {
  ...
  assert.throws(() => encodeItemQrPayload('42'), { code: 'ITEM_QR_INVALID_UUID' });
  assert.throws(() => encodeItemQrPayload(undefined), { code: 'ITEM_QR_INVALID_UUID' });
});
...
test('anything that is not an item code of this version is rejected', () => {
  for (const value of ['', 'ial:item', `http://192.168.1.10:3000/items/${uuid}`, `other:item:v1:${uuid}`,
    ...
    assert.throws(() => decodeItemQrPayload(value), { code: 'ITEM_QR_FOREIGN' }, `accepted ${JSON.stringify(value)}`);
  }
  assert.throws(() => decodeItemQrPayload(`ial:item:v2:${uuid}`), { code: 'ITEM_QR_UNSUPPORTED_VERSION', params: { version: 'v2' } });
  assert.throws(() => decodeItemQrPayload('ial:item:v1:not-a-uuid-at-all-0000-000000000000'), { code: 'ITEM_QR_INVALID_UUID' });
  assert.throws(() => decodeItemQrPayload(`ial:item:v1:${uuid.toUpperCase()}`), { code: 'ITEM_QR_INVALID_UUID' });
});
